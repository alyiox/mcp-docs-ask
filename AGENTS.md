...
* **The tool/parameter description MUST start with [DocsAsk], followed by a Verb-Object fragment**, e.g. `[DocsAsk] Retrieve grounded documentation passages`, `[DocsAsk] Rebuild the local docs index`.
* **Use tag-based lineage (Src: <Entity>) for parameters** (e.g. docs id → Src: config, layer → Src: config, docs path/URL → Src: docs).
* **Every tool MUST declare `ToolAnnotations`**, mapped from the operation it performs:
  ...
  * Set `open_world_hint=True` when the tool reaches the network (e.g. git fetch on `reindex`), `False` when it only reads local index/config
